  ...
  return (
    <div className="mt-8 space-y-6">
      <article className="card flex flex-col items-start justify-between gap-5 bg-[#111a2e] p-6 text-white md:flex-row md:items-center">
        ...
        <a href={apiUrl("/reports/analyses.csv")} className="inline-flex min-h-11 items-center justify-center rounded-xl bg-amber-500 px-5 py-3 text-sm font-black text-slate-950">Download history CSV</a>
      </article>
      <article className="card overflow-hidden">
        ...
        {!records.length ? <div className="p-10 text-center text-sm text-slate-500">Run an analysis to make individual exports available.</div> : (
          <div className="divide-y divide-slate-100">
            {records.map((record) => <div className="flex flex-col justify-between gap-4 p-5 md:flex-row md:items-center" key={record.id}><div><p className="font-bold">Run #{record.id} · {record.dataset}</p><p className="mt-1 max-w-xl truncate text-xs text-slate-500">{record.scenario_id} · {record.vacant_spaces} vacant / {record.occupied_spaces} occupied</p></div><div className="flex gap-2"><a className="inline-flex min-h-11 items-center justify-center rounded-lg border border-slate-300 bg-white px-4 py-2 text-xs font-bold text-slate-900 hover:bg-slate-100" href={apiUrl(`/reports/analyses/${record.id}.csv`)}>CSV</a><a className="inline-flex min-h-11 items-center justify-center rounded-lg bg-slate-900 px-4 py-2 text-xs font-bold text-white" href={apiUrl(`/reports/analyses/${record.id}.json`)}>JSON + slots</a></div></div>)}
          </div>
        )}
      </article>
