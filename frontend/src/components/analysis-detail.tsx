...
export function AnalysisDetail({ analysisId }: { analysisId: string }) {
  ...
  useEffect(() => {
    fetch(`${apiBase}/analysis/history/${encodeURIComponent(analysisId)}`)
      .then(async (response) => { const payload = await response.json(); if (!response.ok) throw new Error(payload.detail ?? "Analysis unavailable"); return payload as Analysis; })
      .then(setAnalysis)
      .catch((reason: Error) => setError(reason.message));
  }, [analysisId]);

  if (error) return <div className="card mt-8 p-8 text-sm font-semibold text-red-600">{error}</div>;
